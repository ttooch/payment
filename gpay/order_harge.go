...
func (oc *OrderCharge) Handle(conf *OrderChargeConf) (interface{}, error) {
...
func (oc *OrderCharge) BuildData(conf *OrderChargeConf) error {
	...
	if oc.ExpireDuration != 0 {
		conf.TimeExpire = time.Now().Add(oc.ExpireDuration).Format("20060102150405")
	} else {
		conf.TimeExpire = ""
	}
